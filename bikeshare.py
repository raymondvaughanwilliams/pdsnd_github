...
def get_city():
    '''Asks the user for a city, get the corresponding bikeshare data and and returns the processed data.
    Args:
        none.
    Returns:
...
def get_time_period():
    ...
    '''
    time_period = ''
    thetimes = ['month','day','none']
    while time_period.lower() not in thetimes:
        time_period = input('\nDo you want to search by day or month'
                            ' Type none to not use a date search.\n')
        if time_period.lower() not in thetimes:
            print('Invalid input.')
    return time_period
...
def get_month():
    ...
    while month_input.lower() not in months_dict.keys():
        month_input = input('\nInvalid Input. Available options are:January,February, March,April, May and June\n')
        if month_input.lower() not in months_dict.keys():
            print('Invalid input. Please type in a '
...
def popular_day(df):
    '''Gets the most popular day of week (Monday, Tuesday, etc.) for start time.
    Args:
        bikeshare dataframe
    Returns:
        none
    ...
    print('The most popular day of week for start time is {}.'.format(popular_day))
...
def popular_hour(df):
    ...
    if most_pop_hour == 0:
        ...
        timeofday = 'am'
        mostpopularhour = most_pop_hour
    elif 13 <= most_pop_hour < 24:
        timeofday = 'pm'
        mostpopularhour = most_pop_hour - 12
    print('The most popular hour of day for start time is {}{}.'.format(mostpopularhour, timeofday))
...
def trip_duration(df):
    ...
    print('The total trip duration is {} hours, {} minutes and {}'
          ' seconds.'.format(hour, mins, secs))
    average_duration = round(df['trip_duration'].mean())
    m, s = divmod(average_duration, 60)
    if m > 60:
        h, m = divmod(m, 60)
        print('The average  duration is {} hours, {} minutes and {}'
              ' seconds.'.format(h, m, s))
    else:
        print('The average  duration is {} minutes and {} seconds.'.format(m, s))
...
def birth_years(df):
    ''' Finds and prints the oldest user, most recent (i.e. 
...
def gender(df):
    ...
    Returns:
        none
    '''
    males = df.query('gender == "Male"').gender.count()
    females = df.query('gender == "Male"').gender.count()
    print('There are {} male users and {} female users.'.format(males, females))
